...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleFileSelect(file) {
        ...
        reader.onload = async function(e) {
            try {
                ...
                log(`อ่านสำเร็จ ${rawData.length} แถว`);
    ...
    async function processExcelData(rawData) {
        ...
        rawData.forEach(row => {
            ...
            if (platform === 'Shopee') { pCode = getValue(row, ['เลขอ้างอิง SKU (SKU Reference No.)', 'Parent SKU', 'SKU']); pName = getValue(row, ['ชื่อสินค้า']); }
        ...
        if (typeof dbSupabase !== 'undefined') {
            ...
            try {
                if (skusToFetch.length > 0) {
                    const { data } = await dbSupabase.from('sku_master').select('*').in('product_code', [...new Set(skusToFetch)]);
                    if (data) data.forEach(item => masterDict['CODE_' + item.product_code.toLowerCase()] = item);
                }
                if (namesToFetch.length > 0) {
                    const { data } = await dbSupabase.from('sku_master').select('*').in('name', [...new Set(namesToFetch)]);
        ...
        rawData.forEach((row, index) => {
            ...
            let orderNumber = `UNK-${index}`, productCode = null, productName = "Unknown", quantity = 1, price = 0, shipping = 0, buyer = "Unknown", phone = null, address = null, province = "Unknown", status = "รอดำเนินการ", note = null;
            if (platform === 'Lazada') {
                ...
                phone = getValue(row, ['billingPhone']);
                ...
                productCode = getValue(row, ['เลขอ้างอิง SKU (SKU Reference No.)', 'Parent SKU', 'SKU']) || productCode;
                ...
                phone = getValue(row, ['เบอร์โทรศัพท์', 'เบอร์โทร']);
                ...
                phone = getValue(row, ['Phone', 'Buyer Phone']);
            ...
            parsedDataToUpload.push({
                ...
                buyer_name: buyer, phone: phone, address: address, province: province, status: status, note: note
            });
        });
        renderPreview();
    }
    ...
    if (bulkUploadBtn) {
        bulkUploadBtn.addEventListener('click', async () => {
            ...
            log(`กำลังเริ่มอัปโหลดชุดใหญ่ ${parsedDataToUpload.length} รายการ...`);
            bulkUploadBtn.disabled = true;
            try {
                const chunkSize = 50;
                for (let i = 0; i < parsedDataToUpload.length; i += chunkSize) {
                    const chunk = parsedDataToUpload.slice(i, i + chunkSize);
                    const { error } = await dbSupabase.from(SUPABASE_TABLE_NAME).insert(chunk);
                    if (error) throw error;
                    log(`อัปโหลดสำเร็จแล้ว ${Math.min(i + chunkSize, parsedDataToUpload.length)} รายการ`);
                }
